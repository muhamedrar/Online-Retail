...
class SalesForecaster:
    ...
    def fittingmodel_with_filtered_country(self, country,cluster):
        trend, seasonal, resid = prep_data_for_sales_forecasting(file_path= data_clustered_path, cluster=cluster, country=country)
        self.trend = trend.dropna()
        self.seasonal = seasonal.dropna()
        self.resid = resid.dropna()
        self.model = ARIMA(self.resid, order=order, seasonal_order=seasonal_order)
